...
-- query name: Launches & Migrations Count
-- query link: https://dune.com/queries/4680980
...
WITH
    ...
    stats_by_day as (
        SELECT
            ...
            SUM(
                case
                    when (label = 'Blum') then 1
                end
            ) AS blum_cnt,
            SUM(
                case
                    when (label = 'BlumMigratedToStonfi') then 1
                end
            ) AS migrated_cnt
        FROM
            labeled_filtered_jettons
...
SELECT
    day,
    blum_cnt,
    migrated_cnt as migrated_cnt,
    SUM(blum_cnt) OVER (
        ORDER BY
            day
    ) AS blum_cnt_total,
    SUM(migrated_cnt) OVER (
        ORDER BY
            day
    ) AS migrated_cnt_total
FROM
    stats_by_day
ORDER BY 
    1